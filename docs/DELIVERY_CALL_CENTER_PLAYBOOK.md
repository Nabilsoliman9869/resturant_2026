...
> **تذكرة دليفري** = بيانات العميل + القناة + الشحن، ثم **جلسة طلب** (فاتورة مبدئية قابلة للتعديل)، وبعد موافقة العميل **تفعيل → مطبخ**، ثم **طابور تسليم** للطيار ثم تسوية نقد/فيزا.

حالات التذكرة (تشغيلياً):

`intake` / `draft_quote` → `quoted` → `kitchen` → `ready` → `out_for_delivery` → `delivered` → `settled` | `cancelled`

> شاشة طلب التوصيل **ليست** جرسون الطاولات — جلسة مستقلة للمبدئية والتفعيل.
...
|---|---|---|
| **كول سنتر** | `/app/cashier/call-center` | يوجّه إلى مركز الدليفري |
| **إدارة الدليفري** | `/app/cashier/delivery-management` | يوجّه إلى مركز الدليفري على تبويب **طابور التسليم** |
| **مركز الدليفري** | `/app/cashier/delivery-hub` | استقبال سريع · التذاكر · تحويل من طاولة · طابور التسليم |
| **شاشة طلب التوصيل** | `/app/cashier/delivery-order` | منيو + سلة + شحن + بحث عميل + الأصناف المحببة (TBL022/023) — ليست نقطة بيع عامة |
...
| تبويب | الاستخدام |
|---|---|
| **استقبال سريع** | واتساب / مكالمة / منصة — بحث عميل بارز — حفظ تذكرة — فتح شاشة الطلب |
| **التذاكر** | كل التذاكر المفتوحة؛ إعادة فتح شاشة طلب التوصيل |
| **من طاولة → دليفري** | ضيف على طاولة يريد توصيل الطلب للمنزل |
| **طابور التسليم** | طلبات بحالة جاهز من المطبخ + بيانات المستلم/الطيار |

رابط داخل المركز: **شاشة طلب التوصيل**.
