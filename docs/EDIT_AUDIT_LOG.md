...
### 156 — مسار الحفظ اليومي / قاعدة التوأم الضريبية — UTC 2026-08-24T02:50:00Z — ID `tax-twin-daily-path-c4f91a`

- **`backend/tax_twin.py`:** حفظ إعداد التوأم، اختبار اتصال، BACKUP/RESTORE أولي تحت مجلد المسار، تريجرات طابور الثوابت، ناشر دفعات فواتير ضريبية (6 ساعات + إعادة ملء الفجوات).
- **`api_server.py`:** مسارات `GET/PUT /api/settings/tax-twin` و`test`/`bootstrap`/`run-publisher` + تشغيل ناشر خلفي عند الإقلاع.
- **`TaxTwinDailySaveSettingsPage.tsx` + إعدادات:** بند «مسار الحفظ اليومي» تحت النظام والتعريفات (مدير/مطوّر).

### 157 — شيكات الكراسي منفصلة + مالك لكل كرسي + نقل بنود — UTC 2026-08-24T13:10:00Z — ID `seat-checks-owner-move-e91a2c`

- **فوترة:** سياسة المالك تُطبَّق فقط على مقاعد `ownerSeatNos`/`seatBillingOverrides` عند تقسيم الكراسي؛ لا تعميم VIP على الكل.
- **كابتن:** معاينة حساب لكل كرسي بقوانينه؛ افتراضي تحديد كل الكراسي غير المفوترة؛ منع «أول محدد = مالك».
- **كاشير:** `awaitingInvoices[]` في نظرة الطاولات — زر تسديد لكل شيك كرسي.
- **`POST …/invoices-local/move-lines`:** نقل بنود بين شيكات نفس الجلسة قبل التسديد + واجهة تصحيح/نقل في شيكات الطاولة.
- **شريحة الطاولة:** إلزام تحديد كرسي المالك قبل تطبيق Owner/VIP.

### 158 — تباين نص نوافذ عمليات الجرسون — UTC 2026-08-28T00:05:00Z — ID `ops-modal-contrast-02b56c82c286`
