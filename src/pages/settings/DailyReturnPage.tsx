...
export default function DailyReturnPage() {
  ...
  async function load() {
    ...
    setRows(Array.isArray(dj.returned) ? dj.returned : []);
  }

  async function save() {
    try {
      const r = await fetch(`${getApiBase()}/api/costing/daily-engine/returns/save`, {
      ...
      const j = JSON.parse(t || "{}"); setMsg(`تم حفظ المسترد آخر اليوم بعدد ${j.written ?? 0} سطر.`);
    } catch (e) { setMsg(`فشل حفظ المسترد: ${String(e)}`); }
  }
  ...
  return (
    <div className="page" style={{ direction: "rtl" }}>
      <h2 style={{ marginTop: 0 }}>شاشة المسترد آخر اليوم</h2>
      ...
      <div className="card">
        <table style={{ width: "100%", borderCollapse: "collapse" }}>
          <thead><tr style={{ background: "rgba(255,255,255,0.04)" }}><th style={{ padding: 6 }}>الصنف الخام</th><th style={{ padding: 6 }}>الكمية</th><th style={{ padding: 6 }}>سعر الوحدة</th><th style={{ padding: 6 }}>التكلفة</th><th style={{ padding: 6 }}>ملاحظة</th><th style={{ padding: 6 }}></th></tr></thead>
          <tbody>
            {rows.map((r, i) => (
              <tr key={`${i}-${r.productGuide}`} style={{ borderTop: "1px solid rgba(255,255,255,0.07)" }}>
                <td style={{ padding: 6 }}><select value={r.productGuide} onChange={(e) => { const pg = e.target.value; const p = options.find((x) => x.CardGuide === pg); const nr = recalc({ ...r, productGuide: pg, productName: p?.ProductName || r.productName, unitCost: Number(p?.Price || r.unitCost || 0) }); setRows((prev) => prev.map((x, ix) => (ix === i ? nr : x))); }}><option value="">— اختر —</option>{options.map((p) => <option key={p.CardGuide} value={p.CardGuide}>{p.ProductName}</option>)}</select></td>
